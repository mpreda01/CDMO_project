...
def create_solver_with_circle_method(n: int, solver_args: dict[str,], constraints: dict[str, bool] =None):
    ...
    for w, week_matches in enumerate(initial_schedule):
        for i, j in week_matches:
            # For each match (i, j) in week w, enforce that it occurs in exactly one period
            match_vars = []
            for p in Periods:
                match_vars.append(And(home[w][p][i], away[w][p][j]))
            solver.add(exactly_one(match_vars, name=f"match_{i}_{j}_week_{w}"))


    # EACH PERIOD HAS EXACTLY ONE HOME TEAM AND ONE AWAY TEAM
    for w in Weeks:
        for p in Periods:
            solver.add(exactly_one([home[w][p][t] for t in Teams], name=f"home_team_week_{w}_period_{p}"))
            solver.add(exactly_one([away[w][p][t] for t in Teams], name=f"away_team_week_{w}_period_{p}"))
