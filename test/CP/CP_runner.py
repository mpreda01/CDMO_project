...
class MinizincRunner:
    ...
    def run_minizinc(self, model_file: str, data_content: str, timeout: int) -> Tuple[bool, str, float, str]:
        ...
        command = [self.minizinc_path, '--solver', 'Gecode', model_file, data_file]
        command.extend(['--random-seed', str(42)])
    ...
    def run_pipeline(self, n: int, params: Dict[str, bool], run_decision: bool, run_optimization: bool, circle_method: bool, skip: Dict[str, bool]) -> List[Dict]:
        ...
        if circle_method:
            ...
            if not skip["skip4"]:
                ...
                if optimization_result['time'] >= self.timeout_seconds:
                    optimization_result['time'] = self.timeout_seconds
                    optimization_result['optimal'] = False
                #print(optimization_result) ## DEBUG
                res.append(optimization_result)
        #print(res) ## DEBUG
        #print("\n")
        #print("\n")
        ...
        try:
            if not combinations:
                ...
                for i, n in enumerate(selected_n, 1):
                    ...
                    pipeline_results = self.run_pipeline(n, params, run_decision, run_optimization, circle_method, skip)
                ...
                circle_combinations = list(itertools.product([True, False], repeat=len(circle_keys)))
                classic_combinations = list(itertools.product([True, False], repeat=len(classic_keys)))
                total_runs = len(selected_n) * len(circle_combinations+classic_combinations)
                pipeline_results = {}
                run_count = 0
                for n in selected_n:
                    for combo in classic_combinations:
                        run_count += 1
                        params = dict(zip(classic_keys, combo))
                        # print(params) ## DEBUG
                        if combinations and run_decision and run_optimization and circle_method:
                            file_path = Path(__file__).parent.parent.parent / "res" / "CP" / f"{n-2}.json"
                        else:
                            file_path = Path(__file__).parent.parent.parent / "personalized_res" / "CP" / f"{n-2}.json"
                        ...
                        if file_path.exists():
                            with open(file_path, "r") as f:
                                data = json.load(f)
                            try:
                                if data[res_name]["stop_reason"] == "time_limit":
                                    pipeline_results.setdefault(n, []).extend({
                        ...
                        if not skip["skip1"] or not skip["skip2"] or not skip["skip3"] or not skip["skip4"]:
                            print(f"[{run_count}/{total_runs}] n={n}")
                            results = self.run_pipeline(n, params, run_decision, run_optimization, circle_method, skip)
                            pipeline_results.setdefault(n, []).extend(
                                results
                            )
                    for combo in circle_combinations:
                        run_count += 1
                        params = dict(zip(circle_keys, combo))
                        # print(params) ## DEBUG
                        if combinations and run_decision and run_optimization and circle_method:
                            file_path = Path(__file__).parent.parent.parent / "res" / "CP" / f"{n-2}.json"
                        else:
                    ...
                    self.save_results(pipeline_results, combinations, run_decision, run_optimization, circle_method)  # Save results
